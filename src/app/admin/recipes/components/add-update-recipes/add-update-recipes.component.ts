...
export class AddUpdateRecipesComponent {
  ...
  getOneRecipe() {
    this.adminService.getOneRecipe(this.recipeId).subscribe({
      next: (res) => {
        this.oneRecipe = res;
        this.apiImageSrc = this.oneRecipe.imagePath;
        console.log(this.apiImageSrc);
        // this.apiImageSrc=this.files[0]
        // this.getImage()
  ...
  // getImage() {
  //  return this.adminService.getImage(this.apiImageSrc).subscribe(
  //   file=>{
  // this.files=[file]
  //  })
  ...
  update() {
    ...
    //   myData.append('recipeImage', this.apiImageSrc)
